...
- E2E production standalone ở 375×667 kiểm route, nav, toggle, tick, console/network và xác nhận `/staff/pos` cùng `/admin/dashboard` không gọi active-order sau hơn 10 giây, còn `/staff/prep` có gọi.
...
Không lệch. Test lifecycle mock các preload hợp lệ của POS/dashboard để mọi response và console trong bằng chứng browser đều sạch.
