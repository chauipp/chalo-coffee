...
### Task 4.5 (xen giữa): BE — ResponseInterceptor bỏ qua route @Sse() [x]

> Task phát sinh trong lúc chạy Task 9 (systematic-debugging). KHÔNG có trong plan gốc nhưng
> bắt buộc để feature realtime chạy thật.

**Bug:** `app.useGlobalInterceptors(new ResponseInterceptor())` (`chalo-be/src/main.ts:67`) bọc
MỌI item Observable của route `@Sse()` thành `{code,message,data:{type,data}}`, đẩy `type` xuống
một tầng. `SseStream._transform` (`@nestjs/core`) cần `type` ở top-level để ghi dòng `event:
<tên>`; mất nó → trình duyệt nhận mọi sự kiện SSE dưới dạng "message" ẩn danh → FE
(`addEventListener(<tên>)`) không bao giờ bắt được. Có từ commit BE đầu (`18f8c3f`), không phải
regression nhánh này, nhưng chặn auto-in / auto-chuyển-trạng-thái.

**Fix (commit `a4e9f0c`):** interceptor nhận diện SSE bằng đúng metadata NestJS tự dùng
(`Reflect.getMetadata(SSE_METADATA, context.getHandler())`) và trả `next.handle()` nguyên bản,
không bọc. Route HTTP thường vẫn giữ envelope `{code,message,data}` như cũ.

**Files:**
- Modify: `chalo-be/src/common/interceptors/response.interceptor.ts`
- Test: `chalo-be/src/common/interceptors/response.interceptor.spec.ts` (mới)

**Verify:** unit test 3/3 pass; full BE suite 68/68 pass; 2 spec e2e phụ thuộc SSE
(`customer-checkout`, `print-station`) chuyển từ FAIL → PASS; full e2e 45/46.

---

### Task 9: E2E — cập nhật spec cũ + spec trạm in mới [x] — ĐÃ ĐÓNG (blocker SSE đã sửa)

> **ĐÃ GIẢI QUYẾT (2026-07-19):** blocker SSE được sửa ở Task 4.5 (commit `a4e9f0c`) —
> `ResponseInterceptor` giờ bỏ qua route `@Sse()` (nhận diện bằng metadata `SSE_METADATA`),
> nên dòng `event: <type>` được ghi lại đúng và FE nghe được sự kiện có tên. Sau khi sửa:
> - Full e2e (`--workers=1`, base `:3001`): **45 passed / 46**. `customer-checkout.spec.ts` và
>   `print-station.spec.ts` (2 spec bắt buộc của Task 9) đều XANH.
> - Fail duy nhất còn lại: `admin-dashboard.spec.ts` — cần dữ liệu doanh thu nhiều ngày để render
>   `.recharts-line-curve`; là lỗ hổng seed/môi trường đã ghi nhận từ đầu, KHÔNG liên quan nhánh này
>   và không do fix interceptor (mọi assert khác của trang đã pass tới bước chart).
> - `admin-orders.spec.ts` + `prep-product-centric.spec.ts` (từng fail vì thiếu seed / cần BE :8082)
>   nay đã pass trong lần chạy này (đủ đơn tích luỹ qua các phiên test / BE phụ sẵn).
...
- [x] **Step 4: Chạy toàn bộ e2e** — XANH sau khi sửa blocker SSE (Task 4.5). 45/46 pass; fail duy nhất `admin-dashboard` là lỗ hổng seed doanh thu ngoài phạm vi (xem hộp trạng thái ở đầu Task 9).
...
### Task 10: Docs — deploy/PRINTING.md + trỏ từ README [x]
...
- [x] **Step 1: Viết deploy/PRINTING.md**
...
- [x] **Step 2: Trỏ từ deploy/README.md**
