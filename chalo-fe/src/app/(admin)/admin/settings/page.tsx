    ...
    <div className="max-w-xl space-y-6 p-4 sm:p-6">
      ...
      <div className="space-y-5 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900 sm:p-5">
        <div>
          ...
          <p className="text-xs text-gray-400">
            Dán API key webhook của SePay. Tiền về đúng nội dung và số tiền sẽ được xác nhận tự động; key đã lưu không được hiển thị lại.
          </p>
        </div>
        ...
        <FormField
          ...
          hint={
            data?.sepayWebhookKeySet
              ? "Đã cấu hình — nhập key mới để thay hoặc gỡ key để tắt."
              : "Chưa cấu hình — nhân viên vẫn có thể xác nhận tay."
          }
        >
          <Input
            type="password"
            ...
            onChange={(event) => setSepayKeyInput(event.target.value)}
          />
        </FormField>
