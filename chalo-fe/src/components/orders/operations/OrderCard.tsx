...
export function OrderCard({ order, onStatusChange, isUpdating, detailHref }: { order: OrderDto; onStatusChange: (orderId: string, status: OrderStatus) => void; isUpdating: boolean; detailHref: (orderId: string) => string }) {
  ...
  return <div role="button" tabIndex={0} draggable={!!nextStatus} onDragStart={(e) => { if (!nextStatus) return; e.dataTransfer.setData("text/plain", order.id); e.dataTransfer.setData(orderDragType(nextStatus), order.id); e.dataTransfer.effectAllowed = "move"; }} onClick={openDetail} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openDetail(); } }} className={`cursor-pointer rounded-xl border-2 bg-white dark:bg-stone-800 shadow-sm p-3.5 space-y-3 hover:shadow-md transition-shadow ${order.paidStatus ? "border-green-400 dark:border-green-600" : "border-red-400 dark:border-red-600"}`}>
    <div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="text-sm font-bold text-stone-900 dark:text-stone-100">{order.tableName}</p><div className="flex items-center gap-1.5"><p className="text-xs text-stone-400 font-mono">#{order.id.slice(-6).toUpperCase()}</p><OrderSourceBadge source={order.orderSource} />{order.orderSource === "POS" && order.pagerNumber != null && <span className="text-[10px] font-semibold text-amber-700 dark:text-amber-300">Thẻ #{order.pagerNumber}</span>}</div></div><div className="text-right shrink-0"><p className="text-xs text-stone-400">{formatAge(now - new Date(order.createdAt).getTime())}</p><span className={`text-[10px] font-semibold ${order.paidStatus ? "text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20" : "text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20"} px-1.5 py-0.5 rounded-full`}>{order.paidStatus ? "Đã thanh toán" : "Chưa thanh toán"}</span></div></div>
