...
import { PaymentMethod, PaymentSource } from './entities/payment-transaction.entity';
...
export class SepayWebhookService {
  ...
  private async process(dto: SepayWebhookDto) {
    ...
    try {
      const completed = await this.orderService.checkoutCompleteStaff(
        { sessionId: session.id, method: PaymentMethod.BANK_TRANSFER },
        undefined,
        PaymentSource.SEPAY,
      );
      if (completed.idempotent) {
        return this.review(base, session, 'Phiên vừa được hoàn tất ở nguồn khác');
      }
    } catch (e) {
